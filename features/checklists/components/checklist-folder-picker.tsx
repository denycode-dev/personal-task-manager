"use client";

import { useState, useTransition, useMemo } from "react";
import { updateChecklistFolderAction } from "@/features/checklists/actions/checklist.action";
import { useConfirm } from "@/lib/hooks/use-confirm";
import { toast } from "sonner";
import { Folder, CircleNotch } from "@phosphor-icons/react";
import type { Folder as FolderType } from "@/lib/db/schema";
import { folderService } from "@/features/folders/services/folder.service";

type Props = {
  checklistId: string;
  currentFolderId: string | null;
  folders: FolderType[];
};

export function ChecklistFolderPicker({ checklistId, currentFolderId, folders }: Props) {
  const [folderId, setFolderId] = useState(currentFolderId ?? "");
  const [isPending, startTransition] = useTransition();
  const confirm = useConfirm();

  const hierarchicalFolders = useMemo(() => {
    return folderService.getFolderHierarchy(folders);
  }, [folders]);

  const handleChange = async (newId: string) => {
    const targetFolder = folders.find((f) => f.id === newId);
    const targetName = targetFolder ? targetFolder.name : "Tanpa folder";

    const ok = await confirm({
      title: "Pindahkan Checklist",
      message: `Pindahkan checklist ini ke "${targetName}"?`,
      confirmLabel: "Pindahkan",
      danger: false,
    });
    if (!ok) return;

    setFolderId(newId);
    startTransition(async () => {
      const result = await updateChecklistFolderAction(checklistId, newId || null);
      if (result.success) {
        toast.success(`Checklist dipindahkan ke "${targetName}".`);
      } else {
        toast.error("Gagal memperbarui folder checklist.");
      }
    });
  };

  if (folders.length === 0) return null;

  return (
    <div className="flex items-center gap-1.5">
      {isPending ? (
        <CircleNotch size={16} weight="bold" className="animate-spin text-neutral-800 shrink-0" />
      ) : (
        <Folder size={16} weight="fill" className="text-neutral-600 shrink-0" />
      )}
      <select
        suppressHydrationWarning
        value={folderId}
        onChange={(e) => handleChange(e.target.value)}
        disabled={isPending}
        className="text-xs border-2 border-black px-2 py-1 bg-white focus:outline-none focus:bg-yellow-50 disabled:opacity-50 font-bold max-w-[150px] sm:max-w-[220px]"
        title="Pindah ke folder"
      >
        <option value="">Tanpa folder</option>
        {hierarchicalFolders.map((f) => (
          <option key={f.id} value={f.id}>
            {f.indentLabel}
          </option>
        ))}
      </select>
    </div>
  );
}

